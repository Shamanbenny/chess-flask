...
namespace Engine.Core.V2;

public static class V2_0Engine
{
    ...
    private const int V2_0AspirationWindow = 60;

    public static SearchResult SearchMoveV2_0(BoardState board, double timeLimitSeconds = 1.0, int? maxDepth = null)
    {
        ...
        int SearchChild(Move move, int remainingDepth, int alpha, int beta, int ply)
        {
            board.Push(move);
            try
            {
                movesEvaluated++;
                return -Negamax(remainingDepth, -beta, -alpha, ply);
            }
            finally
            {
                board.Pop();
        ...
        int Negamax(int remainingDepth, int alpha, int beta, int ply)
        {
            ...
            foreach (var move in OrderedSearchMoves(bestMove))
            {
                ...
                {
                    moveEval = SearchChild(move, remainingDepth - 1, alpha, beta, ply + 1);
                }
                else
                {
                    moveEval = SearchChild(move, remainingDepth - 1, alpha, alpha + 1, ply + 1);
        ...
        bool IsAspirationCandidate(int score)
        {
            return score > -V1_6MateScoreThreshold && score < V1_6MateScoreThreshold;
        }

        (Move? BestMove, int BestEval) SearchRootIteration(
            int depth,
            int alpha,
            int beta,
            Move? preferredMove)
        {
            ...
            foreach (var move in OrderedSearchMoves(preferredMove))
            {
                CheckTime();
                ...
                if (searchedMoveCount == 0)
                {
                    moveEval = SearchChild(move, depth - 1, alpha, beta, 1);
                }
                else
                {
                    moveEval = SearchChild(move, depth - 1, alpha, alpha + 1, 1);
                    if (moveEval > alpha && moveEval < beta)
                    {
                        moveEval = SearchChild(move, depth - 1, alpha, beta, 1);
                    }
            ...
            return (iterationBestMove, iterationBestEval);
        }

        var fallbackMoves = OrderedSearchMoves();
        if (fallbackMoves.Count == 0)
        {
            throw new InvalidOperationException("No legal moves available for v2.0 search");
        }
        ...
        while (!maxDepth.HasValue || depth <= maxDepth.Value)
        {
            ...
            try
            {
                ...
                var preferredMove = rootEntry.BestMove;
                var alpha = V1_6NegativeInfinity;
                var beta = V1_6PositiveInfinity;

                if (completedDepth > 0 && IsAspirationCandidate(bestEval))
                {
                    alpha = bestEval - V2_0AspirationWindow;
                    beta = bestEval + V2_0AspirationWindow;
                }

                var aspirationAlpha = alpha;
                var aspirationBeta = beta;
                (iterationBestMove, iterationBestEval) = SearchRootIteration(depth, alpha, beta, preferredMove);

                if (iterationBestMove is not null
                    && completedDepth > 0
                    && IsAspirationCandidate(bestEval)
                    && (iterationBestEval <= aspirationAlpha || iterationBestEval >= aspirationBeta))
                {
                    (iterationBestMove, iterationBestEval) =
                        SearchRootIteration(depth, V1_6NegativeInfinity, V1_6PositiveInfinity, iterationBestMove);
                }
